...
def device_activity(device, activity):
    ...
    * `severity` - Numberical severity level, which follows the python logging
    ...
    * `category` - The category the action belongs to e.g. "business metric"
    ...
    the action object, `aciton.data["notify"]`, `aciton.data["severity"]` and
    `aciton.data["category"]`
...
def get_all_related_orgs(orgs):
    """ Takes a list of organizaitions and returns a list of these
    organizaitions and all other organizaitions in the parental tree, sorted by
...
def activity_subscription_notfication(action, device):
    ...
    calls the relavent activity notifiers. Each user is only notified once for
    the organization with the lowest parental depth in the organization family
    tree. The function only queries the database once for perforance and sorts
    ...
    for sub in subscriptions:
        ...
        if user_id not in alerted_users or sub.type not in alerted_users[user_id]:
            ...
            try:
                ...
                logger.exception("{} handler failed to send notification to user {}".format(sub.type, user_id))
